...
public class MedicareService {
	...
	public Medicare updateServiceById(int id,Medicare med) throws ValueNotFoundException{
		Optional<Medicare> optionalRecords = medrepo.findById(id);
		
		if(optionalRecords.isPresent()) {
			Medicare existingRecords = optionalRecords.get();
			existingRecords.setServiceName(med.getServiceName());
